...
def compute_rewrite_quality_counterfact(
    ...
    # probs: list of dictionary {"target new":..., "target true":...}
    # targets_correct: list of T/F indications
    ...
    if snips is not None:
        ...
        # text with the same subject and relation
        consistency_texts = [x["text"] for x in snips[rel_id][target_new["id"]]]
        # text with the same subject. relation and object
        essence_texts = [
            x["text"]
...
def test_batch_prediction(
    ...
    with torch.no_grad():
        logits = model(**prompt_tok).logits # (batch_size, sequence_length, vocal_size)
    ...
    for i in range(logits.size(0)):
        ...
        # rewrite/parapharase + new or neighborhood + true (indicate the right target)
        if (which_correct[i // 2] == 0 and i % 2 == 0) or (
            ...
            for j in range(cur_len):
                ...
                if logits[i, prefix_lens[i // 2] + j - 1, :].argmax().item() != cur_tok:
                    # whether the current token has the maximum probability
                    correct = False
                    break
...
if __name__ == "__main__":
    sent = "A frequency distribution for the outcomes of an experiment."
    fdist = compute_freq(sent)
    freqs = np.array([freq for _, freq in fdist.items()])
    freqs = freqs / freqs.sum()
    a = 1